...
exports.patchOne = function(node, fn, data) {
  enterContext(node.parentNode);
  firstChild();
  while (context.walker.currentNode !== node) {
    nextSibling();
  }
  fn(data);
  parentNode();
  clearUnvisitedDOM(node.parentNode);

  if (process.env.NODE_ENV !== 'production') {
    assertNoUnclosedTags(node.parentNode);
  }
